...
export default function ProductDescriptionPage() {
    ...
    useEffect(() => {
        if (id) axios.get(`http://localhost:8080/product/id/${id}`).then((res) => setData(res.data))
    }, [])
        ...
        <div>
...

            {
                ...
                <div className='PorductDetailDiv'>
                    ...
                    <div className='SelectSizeDiv' id='size'><span>Select Size : {selectedSize ? selectedSize : '-'} </span><span><div className='SizesDiv'>
                        <div onClick={() => handleSizeClick(data ? data.sizes[0] : 0)}><p>{data ? data.sizes[0] : 0}</p></div>
                        <div onClick={() => handleSizeClick(data ? data.sizes[1] : 0)}><p>{data ? data.sizes[1] : 0}</p></div>
                        <div onClick={() => handleSizeClick(data ? data.sizes[2] : 0)}><p>{data ? data.sizes[2] : 0}</p></div>
                        <div onClick={() => handleSizeClick(data ? data.sizes[3] : 0)}><p>{data ? data.sizes[3] : 0}</p></div>
                        <div onClick={() => handleSizeClick(data ? data.sizes[4] : 0)}><p>{data ? data.sizes[4] : 0}</p></div>
                    </div></span></div>
